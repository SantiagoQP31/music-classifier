...
export function ResultPanel({ result, onReset }: ResultPanelProps) {
    ...
    return (
        <section className="result-panel" style={{ '--genre-color': info.color } as React.CSSProperties}>
          <div className="result-panel__inner">
    
            <div className="result-panel__genre">
            ...
            <div className="result-panel__tracks">
              ...
              <ul className="result-panel__track-list">
                {info.tracks.map((track) => (
                  <li key={`${track.artist}-${track.title}`} className="result-panel__track">
                    <span className="result-panel__track-dot" />
                    <div className="result-panel__track-info">
                      <span className="result-panel__track-title">{track.title}</span>
                      <span className="result-panel__track-artist">{track.artist}</span>
                    </div>
                  </li>
                ))}
              </ul>
